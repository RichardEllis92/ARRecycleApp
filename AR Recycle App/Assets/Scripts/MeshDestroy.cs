    ...
    */
    ...
    private void DestroyMesh(GameObject Recycle)
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            parts[i].Recycle.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * ExplodeForce, transform.position);
        }
    ...
    public class PartMesh
    {
        ...
        {
            Recycle = new GameObject(original.name);
            Recycle.transform.position = original.transform.position;
            Recycle.transform.rotation = original.transform.rotation;
            Recycle.transform.localScale = original.transform.localScale;
            ...
            var renderer = Recycle.AddComponent<MeshRenderer>();
            renderer.materials = original.GetComponent<MeshRenderer>().materials;

            var filter = Recycle.AddComponent<MeshFilter>();
            filter.mesh = mesh;

            var collider = Recycle.AddComponent<MeshCollider>();
            collider.convex = true;

            var rigidbody = Recycle.AddComponent<Rigidbody>();
            var meshDestroy = Recycle.AddComponent<MeshDestroy>();
